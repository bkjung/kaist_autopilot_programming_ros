...
#include "kaist_gazebo/kaist_drive.h"

Turtlebot3Drive::Turtlebot3Drive()
  : nh_priv_("~")
{
  //Init gazebo ros kaist node
...
{
  static uint8_t kaist_state_num = 0;
  double wheel_radius = 0.033;
  double kaist_rotation = 0.0;

  kaist_rotation = (rotate_angle_ * turning_radius_ / wheel_radius);

  switch(kaist_state_num)
  {
    case GET_TB3_DIRECTION:
      if (direction_vector_[CENTER] > front_distance_limit_)
      {
        kaist_state_num = TB3_DRIVE_FORWARD;
      }

      if (direction_vector_[CENTER] < front_distance_limit_ || direction_vector_[LEFT] < side_distance_limit_)
      {
        priv_right_joint_encoder_ = right_joint_encoder_ - kaist_rotation;
        kaist_state_num = TB3_RIGHT_TURN;
      }
      else if (direction_vector_[RIGHT] < side_distance_limit_)
      {
        priv_right_joint_encoder_ = right_joint_encoder_ + kaist_rotation;
        kaist_state_num = TB3_LEFT_TURN;
      }
      break;

    case TB3_DRIVE_FORWARD:
      updatecommandVelocity(LINEAR_VELOCITY, 0.0);
      kaist_state_num = GET_TB3_DIRECTION;
      break;
...
{
  ros::init(argc, argv, "kaist_drive");
  Turtlebot3Drive kaist_drive;

  ros::Rate loop_rate(125);

  while (ros::ok())
  {
    kaist_drive.controlLoop();
    ros::spinOnce();
    loop_rate.sleep();
  }
