...
#include <kaist_msgs/SensorState.h>
#include <kaist_msgs/VersionInfo.h>
...
void sensorStateMsgCallback(const kaist_msgs::SensorState::ConstPtr &msg)
{
  if (msg->battery > 11.0)
  ...
  if (msg->button == kaist_msgs::SensorState::BUTTON0)
    setButtonDiagnosis(diagnostic_msgs::DiagnosticStatus::OK, "BUTTON 0 IS PUSHED");
  else if (msg->button == kaist_msgs::SensorState::BUTTON1)
...
void versionMsgCallback(const kaist_msgs::VersionInfo::ConstPtr &msg)
{
  ...
  if (check_version == false)
  {
    if (std::string(msg->software) != std::string(SOFTWARE_VERSION))
      ROS_WARN("Check kaist repository and Update your software!!");

    if (std::string(msg->hardware) != std::string(HARDWARE_VERSION))
      ROS_WARN("Check kaist wiki page and Update your hardware!!");
...
{
  ros::init(argc, argv, "kaist_diagnostic");
  ros::NodeHandle nh;
